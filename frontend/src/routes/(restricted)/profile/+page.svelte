<script lang="ts">
	...
	let formData = $state({
		...
		preferences: (userInfo?.preferences || []) as string[]
	});
	...
	let openPreferencesPopover = $state(false);
	let avatarPreview = $state(
		userInfo?.avatar
			? `${pb.baseURL}/api/files/${userInfo.collectionId}/${userInfo.id}/${userInfo.avatar}`
			: ''
	);
	let cvPreview = $state(
		userInfo?.cv
			? `${pb.baseURL}/api/files/${userInfo.collectionId}/${userInfo.id}/${userInfo.cv}`
			: ''
	);
	...
	onMount(async () => {
		try {
			...
			const preferencesField = userInfoCollection.schema.find(
				(field) => field.name === 'preferences'
			);
			if (preferencesField) {
	...
	function togglePreference(preferenceToToggle: string) {
		const index = formData.preferences.indexOf(preferenceToToggle);
		if (index > -1) {
			formData.preferences.splice(index, 1);
		} else {
			formData.preferences.push(preferenceToToggle);
		}
	}
		...
		<div class="lg:col-span-1 space-y-6">
			...
			<Card.Root>
				<Card.Content class="flex flex-col items-center space-y-4 p-6">
					<div class="relative">
						<Avatar.Root
							class="h-32 w-32 cursor-pointer hover:opacity-80 transition-opacity"
							onclick={handleAvatarClick}
						>
							<Avatar.Image src={avatarPreview} alt="Profile picture" />
			...
			<Card.Root>
				...
				<Card.Content>
					<div class="flex items-center space-x-3">
						<!-- Status Indicator -->
						<div
							class="flex items-center justify-center w-8 h-8 rounded-full {user.approved
								? 'bg-green-100'
								: 'bg-yellow-100'}"
						>
							{#if user.approved}
				...
				<Card.Content>
					...
					<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
						...
						<div class="space-y-2">
							...
							<Input id="email" type="email" value={user.email} disabled class="bg-gray-50" />
			...
			<Card.Root>
				...
				<Card.Content>
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<!-- Update Button -->
						<Button onclick={handleUpdate} disabled={isUpdating} class="w-full">
							{isUpdating ? 'Updating...' : 'Update Profile'}
						</Button>

						<!-- Logout Button -->
						<Button variant="outline" onclick={handleLogout} disabled={isLoggingOut} class="w-full">
